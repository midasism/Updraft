...
final class SelfUpdateRecoveryTests: XCTestCase {
    ...
    private func makeBundle(at url: URL, version: String) throws -> URL {
        ...
        let plist: [String: Any] = [
            ...
            "CFBundleExecutable": "Updraft",
            "CFBundleShortVersionString": version
        ]
        try PropertyListSerialization
            .data(fromPropertyList: plist, format: .xml, options: 0)
            .write(to: contents.appendingPathComponent("Info.plist"))
        let executable = macOS.appendingPathComponent("Updraft")
    ...
    func testNewInstanceCleansDisplacedOldBundle() throws {
        try makeBundle(at: root.appendingPathComponent("Updraft.app"), version: "0.3.0")
        try makeBundle(
            at: root.appendingPathComponent(".Updraft.\(token).old.app"),
            version: "0.2.2"
        )

        let report = Installer.recoverInterruptedInstalls(in: [root])
        XCTAssertTrue(report.rescuedApps.isEmpty)
        XCTAssertEqual(report.removedArtifacts, [".Updraft.\(token).old.app"])
        XCTAssertEqual(
            Installer.plistValue("CFBundleShortVersionString", in: root.appendingPathComponent("Updraft.app")),
            "0.3.0"
        )
    }

    func testRescuesUpdraftWhenSwapWasInterrupted() throws {
        try makeBundle(
            at: root.appendingPathComponent(".Updraft.\(token).old.app"),
            version: "0.2.2"
        )
        ...
        let report = Installer.recoverInterruptedInstalls(in: [root])
        XCTAssertEqual(report.rescuedApps, ["Updraft"])
        let restored = root.appendingPathComponent("Updraft.app")
    ...
    func testKeepsArtifactsWhenNothingCanRescueUpdraft() throws {
        try makeBundle(
            at: root.appendingPathComponent(".Updraft.\(token).new.app"),
            version: "0.3.0"
        )
        let report = Installer.recoverInterruptedInstalls(in: [root])
        XCTAssertTrue(report.rescuedApps.isEmpty)
    ...
    func testRelaunchForcesNewInstanceInsteadOfActivatingRunningOne() {
        let app = URL(fileURLWithPath: "/Applications/Updraft.app")
        let command = Installer.relaunchCommand(for: app)
        XCTAssertEqual(command.executable, "/usr/bin/open")
        XCTAssertEqual(command.arguments, ["-n", "/Applications/Updraft.app"])
    }
}
...
final class SelfUpdateIdentityTests: XCTestCase {
    func testMainListExcludesSelf() {
        let selfApp = SelfUpdateIdentity.makeAppInfo(
            at: URL(fileURLWithPath: "/Applications/Updraft.app"),
            version: "0.2.2"
        )
        let other = AppInfo(
            name: "IINA",
