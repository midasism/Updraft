...
public struct Installer: Sendable {
    ...
    private func validate(
        ...
        guard let stagedBundleID = PackageExtractor.bundleIdentifier(of: stagedApp), stagedBundleID == bundleID else {
            throw InstallError.validationFailed(
                "安装包的 Bundle ID 是 \(PackageExtractor.bundleIdentifier(of: stagedApp) ?? "空")，与目标应用 \(bundleID) 不一致"
            )
        }

        let stagedVersion = PackageExtractor.plistValue("CFBundleShortVersionString", in: stagedApp) ?? advertisedVersion
    ...
    private func verifyInstalled(path: URL, bundleID: String, expectedVersion: String) async throws {
        ...
        guard PackageExtractor.bundleIdentifier(of: path) == bundleID else {
            throw InstallError.validationFailed("替换后的 Bundle ID 不正确")
        }
        let version = PackageExtractor.plistValue("CFBundleShortVersionString", in: path)
...
extension Installer {
    ...
    static func isHealthyBundle(_ app: URL) -> Bool {
        ...
        guard let bundleID = PackageExtractor.plistValue("CFBundleIdentifier", in: app), !bundleID.isEmpty,
              let executable = PackageExtractor.plistValue("CFBundleExecutable", in: app), !executable.isEmpty else {
            return false
        }
