...
public struct Installer: Sendable {
    ...
    ///
    /// - Parameter existing: 磁盘上当前那个包。默认就是 `target`；两者不同只发生在自更新
    ///   改名的过渡期——旧包还在 `/Applications/AppUpdater.app`，而新包要落到
    ///   `/Applications/Updraft.app`。此时**必须把旧包也一并挪走**，否则用户会看到
    ///   同名的两个 App。
    private func swapIn(
        newApp: URL,
        at target: URL,
    ...
    /// 换包之后的失败回滚。
    ///
    /// 回滚把旧包放回 `target`（新名字）。改名过渡期里旧包原本叫别的名字，
    /// 这一下就落到新名字上了——对一个"刚刚换包失败"的用户来说，
    /// 拿到一个名字新、版本旧但**能用**的 App，比拿到一个半新半旧的目录强。
